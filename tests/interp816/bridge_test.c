 ...
 *   S12: runtime-call fallback executing M=0 PLA; RTL consumes its inner JSR
...
int main(void) {
    ...
    /* S12: a runtime-pointer JSR falls back to the interpreter at a state
    ...
    { memset(RAM, 0, MEMSZ); init_cpu(); g_post_return_skip = 0;
      ...
      printf("S12 runtime call propagates interpreted PLA; RTL NLR\n");
      CHECK(r == RECOMP_RETURN_SKIP_1, "r=%d exp SKIP_1", (int)r);
      CHECK(g_c.S == 0x01FF,
